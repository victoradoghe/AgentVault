 ...
 *
 *   AMC_API_KEY   (required)  — the user's Agent Memory Cloud API key.
 *   AMC_BASE_URL  (optional)  — base URL of the AMC REST API.
 *                               Defaults to the hosted deployment; override
 *                               with http://localhost:3000 for local dev.
 */

/** Default hosted AMC deployment. Override with AMC_BASE_URL for local dev. */
  ...
  if (!apiKey) {
    throw new Error(
      "AMC_API_KEY is not set. Add it to your agent's MCP server config " +
        ...
        "Agent Memory Cloud dashboard under Settings → API Keys.",
    );
  }
